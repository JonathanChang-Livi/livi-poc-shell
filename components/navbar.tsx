...
const NavTab = ({ tab, current, title }: NavTabProps) => {
    return (
        ...
        <Title className={`w-full p-4 whitespace-nowrap text-md font-black ${tab === current ? 'text-primary' : 'text-slate-300'} hover:text-primary hover:border-r-4 border-primary ${tab === current ? 'border-r-4 ' : ''}}`}>{title}</Title>
        // </div>
    )
}
...
const NavBar = ({ tab }: NavbarProps) => {
    return (
        <Card override className="mr-2 w-1/4 p-2 pt-5">
